...
$('document').ready(function() {
    ...
    Progressbar.Progress = function(data) {
        ...
        $(".progress-container").fadeIn('fast', function() {
            $("#progress-bar").stop().css({"width": 0, "background-color": "#6499ed"}).animate({
              width: '100%'
            }, {
    ...
    Progressbar.ProgressCancel = function() {
        $("#progress-label").text("Прекратено...");
        $("#progress-bar").stop().css( {"width": "100%", "background-color": "rgba(120, 0, 0, 0.8)"});
        $('#progress-bar').removeClass('cancellable');
